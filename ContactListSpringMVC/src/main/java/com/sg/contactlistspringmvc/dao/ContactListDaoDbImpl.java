...
public class ContactListDaoDbImpl implements ContactListDao {
    ...
    public Contact addContact(Contact contact) {
        ...
        //query database for the id that was just assigned to the new row
        int newId = jdbcTemplate.queryForObject("select LAST_INSERT_ID()", Integer.class);

        //set the new id value on contact object
    ...
    public Contact getContactById(long contactId) {
        try {
            ...
            //no results for given contact id so return null
            return null;
        }
    }
    ...
    public List<Contact> searchContacts(Map<SearchTerm, String> criteria) {
        if (criteria.isEmpty()) {
            ...
            //build a prepared statement based on user search terms
            ...
            //put positional parameters into an array
            //order of parameters matches order search criteria is retrieved
            //from map
            String[] paramVals = new String[numParams];
            Set<SearchTerm> keySet = criteria.keySet();
            Iterator<SearchTerm> iter = keySet.iterator();
            //build up the where clause based on key/value pairs in the map
            while (iter.hasNext()) {
                SearchTerm currentKey = iter.next();
                //if we are not the first one in, we must add an AND
                //to the where clause
                if (paramPosition > 0) {
                    sQuery.append(" and ");
                }
